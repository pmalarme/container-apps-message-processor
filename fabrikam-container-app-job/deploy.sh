...
RESOURCE_GROUP_NAME=${RESOURCE_GROUP_NAME:-"rg-fabrikam-job"}
LOCATION=${LOCATION:-"eastus"}
ENVIRONMENT_NAME=${ENVIRONMENT_NAME:-"test"}

CONTAINER_REGISTRY_NAME=${CONTAINER_REGISTRY_NAME:-"crfabrikamjob$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 5 | head -n 1)"}
CONTAINER_APP_NAME=${CONTAINER_APP_NAME:-"ca-message-processor-job"}
SERVICE_BUS_NAMESPACE_NAME=${SERVICE_BUS_NAMESPACE_NAME:-"sbns-fabrikam-job$(cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 5 | head -n 1)"}
SERVICE_BUS_QUEUE_NAME=${SERVICE_BUS_QUEUE_NAME:-"sbq-fabrikam-job"}
CONTAINER_APP_ENVIRONMENT=${CONTAINER_APP_ENVIRONMENT:-"cae-fabrikam-job"}
LOG_ANALYTICS_WORKSPACE_NAME=${LOG_ANALYTICS_WORKSPACE_NAME:-"log-fabrikam-job"}

DEPLOYMENT_NAME=${DEPLOYMENT_NAME:-"fabrikam-job-main-deployment"}
CONTAINER_APP_DEPLOYMENT_NAME=${CONTAINER_APP_DEPLOYMENT_NAME:-"fabrikam-job-containerapp-deployment"}

IMAGE_NAME=${IMAGE_NAME:-"fabrikam-job-example"}
...
mvn clean install -D"job.container-image.build"=true -DskipTests=true > /dev/null 2>&1
